...
public sealed class FixturesDbContextTests : IDisposable
{
    ...
    public async Task SaveChangesAsync_PersistsVenueAndOutboxMessageAtomically()
    {
        ...
        outbox.Type.Should().Be(FixturesOutboxTypeKeys.VenueCreatedDomainEvent);
        outbox.Status.Should().Be(OutboxMessageStatus.Pending);
        venue.DomainEvents.Should().BeEmpty();
    }

    [Fact]
    public async Task SaveChangesAsync_WithCoordinates_ShouldNotPersistOutboxMessage()
    {
        var venue = Venue.CreateFromProvider(
            "Wembley",
            "TheSportsDB",
            "venue-2",
            Address.Create(null, "London", "England").Value,
            DomainCoordinates.Create(51.5557, -0.2795).Value).Value;

        _dbContext.Venues.Add(venue);
        await _dbContext.SaveChangesAsync();

        (await _dbContext.OutboxMessages.CountAsync()).Should().Be(0);
        venue.DomainEvents.Should().BeEmpty();
    }
